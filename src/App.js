...
export default function App() {
  ...
  // Celo Sepolia RPC pour lecture seule
  const CELO_RPC = "https://forno.celo-sepolia.celo-testnet.org/";
  const CONTRACT_ADDRESS = "0x9ca48c428000a4abd84fb60b22c74e2bf7d65323"; // Remplace par ton contrat
  const web3ReadOnly = new Web3(CELO_RPC);
  const readOnlyContract = new web3ReadOnly.eth.Contract(CeloClickerABI, CONTRACT_ADDRESS);
  ...
  const connectWallet = async () => {
    if (window.ethereum) {
      try {
        ...
        console.error("Erreur connexion wallet :", e);
      }
    } else {
      alert("Veuillez installer MetaMask ou un wallet compatible Celo");
    }
  };
  ...
  const fetchLeaderboard = async () => {
    try {
      const data = await (contract || readOnlyContract).methods.getLeaderboard().call();
